...
class HiddenLayer(DeepGPLayer):
    ...
    def __init__(self, input_dims, output_dims, covar_module, num_inducing=128):
    ...
    def get_sigma2(self):
        return self.covar_module.outputscale.data

    def get_lengthscale(self):
        return self.covar_module.base_kernel.lengthscale.data
...
class DeepGPRegression(DeepGP):
    ...
    def __init__(self, input_dims, n_layer=5, inter_dims=20):
        ...
        covar_module = ScaleKernel(
            RBFKernel(ard_num_dims=None), ard_num_dims=None
        )
        ...
        if n_layer == 2:
            ...
            self.hiddens = torch.nn.ModuleList(
                [HiddenLayer(input_dims=inter_dims,
                             output_dims=inter_dims,
                             covar_module=covar_module) for _ in range(n_layer - 2)])
...
# # save dir
save_dir = "../data/dgp_regression_models_share_kernel"
...
for i, d in enumerate(tqdm.tqdm(dims, desc='Dimension')):
    for j, n_layer in enumerate(tqdm.tqdm(n_layers, desc='Layer')):
        ...
        train(model, train_loader, n_iter=50)
...
from src.plot_utils import plot_dual_ez_rmse_by_dimension
selected_dim=9
save_file = "../figure/experiment_boston/sharing/m_{}_dgp.png".format(selected_dim)
